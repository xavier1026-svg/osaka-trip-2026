...
import { MapPin, Train, Utensils, ShoppingBag, BedDouble, Camera, Plane, Navigation } from 'lucide-react';

interface Props {
  items: ItineraryItem[];
}

const ItineraryTimeline: React.FC<Props> = ({ items }) => {
  ...
  // Industrial Palette: Black, White, Orange, Gray
  const getCardStyle = (type: ActivityType) => {
    switch (type) {
      case ActivityType.Food: return 'bg-white';
      case ActivityType.Shopping: return 'bg-white';
      case ActivityType.Sightseeing: return 'bg-ind-orange text-ind-black'; // Highlight sightseeing
      default: return 'bg-white';
    }
  };

  return (
    <div className="space-y-6 relative ml-2 my-4">
        {/* Continuous Line */}
      ...
      {items.map((item) => (
        <div key={item.id} className="relative pl-12">
          ...
          {/* Content Card */}
          <div className={`p-4 border-ind shadow-hard ${getCardStyle(item.type)} transition-transform active:translate-x-[2px] active:translate-y-[2px] active:shadow-none`}>
            <div className="flex justify-between items-start mb-2 border-b-2 border-ind-black/10 pb-2">
              ...
              {item.type === ActivityType.Transport && (
                <span className="text-[10px] font-bold border-2 border-ind-black px-1 uppercase bg-ind-gray">
